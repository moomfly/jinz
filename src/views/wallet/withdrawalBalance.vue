<template>
  <div>
    <nav-bar title="余额提现"></nav-bar>
    <div class="withdrawal-balance">
      <van-form  ref="form" @submit="onSubmit" validate-trigger="onSubmit">
        <van-field v-model="form.balance" name="balance" label="余额"  readonly>
          <template #button>
            <van-button size="small" type="info" native-type="button"  @click="gotoAll">全部</van-button>
          </template>
        </van-field>
        ...
        <div class="tip-box">
          ...
          <div>2、提现手续费{{user_balance_cash_cate}}%</div>
...
export default {
  ...
  methods:{
    ...
    changeAmount(val){
      let lixi= val* (this.user_balance_cash_cate / 100)
      lixi = Math.ceil(lixi * 100)/100
    ...
    validateCashAmount(val) {
      return val <= Number(this.form.balance);
    },
    minCashAmount(val) {
      return val > 0
    }
