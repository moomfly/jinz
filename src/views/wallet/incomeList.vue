<template>
  <div class="balance-list">
    <nav-bar title="佣金记录"></nav-bar>
    <refresh-list :freshMap="freshMap"  @refresh="onRefresh" @onLoad="onLoad" :hasList="listData.length > 0?true:false">
      <div class="header-box">
        <div class="money">{{ infoData?.income }}</div>
        <div style="font-size: 12px;">可提现佣金(元)</div>
        <van-button type="danger" plain class="header-btn" :disabled="Number(infoData?.income) <= 0"
                    color="#ff6034" to="/withdrawalIncome">提现</van-button>
      </div>
      <van-sticky offset-top="48">
        <van-tabs @change="changeTab" title-inactive-color="#a7a7a7">
          <van-tab title="佣金流水" name="1"></van-tab>
...
<script>
import { cashRecord, incomeRecord} from "@/api/wallet";
export default {
  data() {
    return {
      freshMap:{
  ...
  methods:{
    ...
    getList(){
      let urlApi=this.activeId == 1?incomeRecord:cashRecord
