...
<!--      <van-field v-model="form.mobile" name="mobile" label="营业时间"  placeholder="填写营业时间"
      ...
      <div style="padding:30px 16px;display: flex">
        <van-button block type="danger" native-type="submit" :loading="loading" :round="true"  @click="onSubmit('apply', true)"
                    color="#f78542" >
          修改信息
        </van-button>
      </div>
...
import { merchantApplyDetail, merchantEdit} from "@/api/merchant";
...
export default {
  ...
  methods: {
    ...
    async submitForm(valid) {
      ...
      data.open_time = timeRange[0].trim() || ''
      data.close_time = timeRange[1].trim() || ''
